...
document.addEventListener("DOMContentLoaded", () => {
    const container = document.getElementById("cardGlass");
    if (!container) return;
    ...
    function createLetter() {
        ...
        // 1. Tentukan ukuran acak
        const w = Math.random() * 30 + 40; // lebar 40-70px
        const h = w * 0.7;                // tinggi proporsional

        letter.style.width = w + "px";
        letter.style.height = h + "px";

        // 2. Atur Segitiga secara manual lewat CSS Variables
        // Kita set border-top (tinggi segitiga) dan border-left/right (setengah lebar)
        const triangleHeight = h * 0.5;
        const halfWidth = w / 2;

        // Kita inject style langsung ke tag <style> khusus elemen ini
        letter.style.setProperty('--th', triangleHeight + 'px');
        letter.style.setProperty('--hw', halfWidth + 'px');
        
        // Hubungkan ke pseudo-element via inline style (trik jitu)
        const style = document.createElement('style');
        style.innerHTML = `
            .letter-id-${Date.now()}::before {
                border-top-width: ${triangleHeight}px;
                border-left-width: ${halfWidth}px;
                border-right-width: ${halfWidth}px;
            }
        `;
        // Cara yang lebih simpel: Gunakan CSS variables yang diakses .letter::before
        letter.style.setProperty('--th', triangleHeight + 'px');
        letter.style.setProperty('--hw', halfWidth + 'px');

        // Sisa logika posisi & animasi
        letter.style.right = Math.random() * 60 + "%";
        letter.style.animation = `fallLetter ${Math.random() * 4 + 6}s linear forwards`;
