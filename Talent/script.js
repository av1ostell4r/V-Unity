...
const characters = {
  ...
  seiko: {
    ...
    type:       "MESSENGER BIRD",
    name:       "Seiko Hato",
    alias:      "Nickname: Seiko / Hato",
    nick:       "Seiko / Hato",
    lang:       "ID / JP",
    debut:      "15 September 2025",
    bday:       "20 May",
    story:      "Seorang gadis yang bekerja sebagai pengirim pesan antar dimensi, seringkali tersesat namun selalu sampai tepat waktu.",
    avatar:     "Seiko/Seiko 1.png",
    debutVid:   "dQw4w9WgXcQ", // Placeholder
...
function initWaves() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    w.className = "wave";
    w.style.bottom = `${i * 15}px`;
    w.style.opacity = 0.05 + (i * 0.05);
    w.style.animationDuration = `${6 + (i * 2)}s`;
    w.style.background = `linear-gradient(to top, var(--accent), transparent)`;
    bg.appendChild(w);
  }
}
...
function switchCharacter(charId) {
  ...
  const ytLink  = document.getElementById("debut-link");
  if (ytThumb) ytThumb.src = `https://i.ytimg.com/vi/${c.debutVid}/maxresdefault.jpg`;
  if (ytLink)  ytLink.href = `https://www.youtube.com/watch?v=${c.debutVid}`;
  ...
  document.querySelectorAll(".icon").forEach(icon => {
    ...
    if (key && c.socials[key]) {
      icon.onclick = () => window.open(c.socials[key], "_blank");
      // Update social icon images if necessary (assuming they are in the char folder)
      const folder = c.avatar.substring(0, c.avatar.lastIndexOf("/"));
      // icon.src = `${folder}/${key.toUpperCase()}.png`;
    }
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (avatar && container) {
    ...
    window.addEventListener("mouseup", () => { isDragging = false; if (scale > 1) avatar.style.cursor = "grab"; });
  ...
  if (charParam && characters[charParam]) {
    switchCharacter(charParam);
  } else {
    switchCharacter("tako");
  }
});
